...
# Импортируем таймер
import time
# Импортируем поточность для создания многопоточности
...
def book_car(request, car_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Временная зона UTC+3 для получения текущего  времени
            offset = datetime.timezone(datetime.timedelta(hours=3))
            # Время окончания бронирования
            t_end = booking.end_time
            # Текущее время
            t_now = datetime.datetime.now(offset)
            # Получение оставшегося времени из меток (т.к. разница datetime выдаёт неверный результат)
            remeaning_time = (int((time.mktime(t_end.timetuple()) + t_end.microsecond / 1000000.0) * 1000) - int((time.mktime(t_now.timetuple()) + t_now.microsecond/1000000.0) * 1000))

            # Запуск процесса таймера
            Timer(remeaning_time/1000, car_book_delete, (booking,)).start()
...
# Метод возвращения доступности машины для бронирования
def car_book_delete(booking):
    try:
        car = booking.car
